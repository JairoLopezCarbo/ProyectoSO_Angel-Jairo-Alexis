...
void GenerateSyllable(char silaba[MAX_NAME], int tipo) {
    char consonantes [21] = "bcdfghjklmnpqrstvwxyz";  // Consonantes
	
    char vocales [5] = "aeiou";  // Vocales
    char diptongos[12][2] = {"ai", "ei", "oi", "ui", "au", "eu", "ia", "ie", "io", "ua", "ue", "uo"};  // Diptongos
/*	
*/    // Declaramos una variabl�e para almacenar la sílaba generada
    char aux[MAX_NAME];
    int count = 0;  // Variable para verificar cuántas veces aparece la sílaba
    // Bucle para intentar generar una sílaba que cumpla la condición
    do {
        // Generar una sílaba aleatoria según el tipo seleccionado
        if (tipo == 0) {  // Caso 1: Consonante + Vocal (CV)
            ...
            sprintf(aux, "%c%c", consonante, vocal);  // Asignar la sílaba generada
        }
        else if (tipo == 1) {  // Caso 2: Vocal + Consonante (VC)
            char vocal = vocales[rand() % (sizeof(vocales) / sizeof(vocales[0]))];
            char consonante = consonantes[rand() % (sizeof(consonantes) / sizeof(consonantes[0]))];
            sprintf(aux, "%c%c", vocal, consonante);  // Asignar la sílaba generada
        }
        else if (tipo == 2) {  // Caso 3: Consonante + Vocal + Consonante (CVC)
            ...
            sprintf(aux, "%c%c%c", consonante1, vocal, consonante2);  // Asignar la sílaba generada
        }
        else {  // Caso 4: Diptongo (2 vocales)
            char *diptongo = diptongos[rand() % (sizeof(diptongos) / sizeof(diptongos[0]))];
            sprintf(aux, "%s", diptongo);  // Asignar el diptongo generado
        }

        // Comprobar la existencia de la sílaba en el diccionario
        char query[MAX_QUERY];
        sprintf(query, "SELECT COUNT(word) FROM Dictionary WHERE word LIKE '%%%s%%';", aux);
        SelectResult result = SelectQuery(query);  // Ejecutar la consulta SQL y obtener el número de coincidencias
		
		// Asegurarse de que `result.data[0]` contiene el valor adecuado (debe ser un string que represente un número)
		count = atoi(result.data[0][0]);  // Convertir la cadena resultante en un número entero



    } while (count < 75);

    // Asignar la sílaba generada a la variable de salida (silaba)
